...
export default function ListItem({itemObj, itemType, onPress}: any){
    ...
    return(
        <TouchableOpacity style={{marginBottom:10}} onPress={onPress}>
            {(itemType !== "add") && <View style={{width: '100%', height: 'auto', backgroundColor: '#fff', flexDirection: 'row', borderColor:"#FB4D27", borderWidth: 1, borderRadius:15}}>
                <Image
                    source={icon}
            ...
            {(itemType === "add") && 
                <View style={{width: '100%', height: 70, backgroundColor: '#FB4D27', flexDirection: 'row', borderColor:"#FB4D27", borderWidth: 1, borderRadius:15}}>
                    <View style={{margin:'auto', flexDirection: 'row'}}> 
                        <Image
                            source={icon}
                            style={{alignSelf: 'center', marginLeft: 15, marginRight: 15}}
                        />
                        <Text style={styles.bodyTextWhite}> Add {itemObj.addType === "record"? "Record" : "Pet"}</Text>
                    </View>
                </View>}
        </TouchableOpacity>
    )
